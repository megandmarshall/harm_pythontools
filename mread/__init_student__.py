...
def setpythonpath():
    ...
    if os.path.exists("py") == 1:
        ...
        print(
            "No pythonpath found, assuming PYTHONPATH=%s will work."
...
def setmplconfigpath(uniquenum=None):
    ...
    try:
        ...
        print("Original MPLCONFIGDIR=%s" % (origmplconfigdir))
        sys.stdout.flush()
        mycwd = origmplconfigdir + "%d" % (uniquenum)
        print("New MPLCONFIGDIR=%s" % (mycwd))
        sys.stdout.flush()
    except KeyError:
        print("No original MPLCONFIGDIR")
        sys.stdout.flush()
    ...
    try:
        ...
        try:
            os.makedirs(mycwd)
            print("try mkdir for MPLCONFIGDIR=%s" % (mycwd))
            sys.stdout.flush()
        except:
            print(
                "Problem: matplotlib import didn't work, but directory couldn't be made"
            )
            sys.stdout.flush()
            print("os.path.exists: %d" % (os.path.exists(mycwd)))
            sys.stdout.flush()
    #
...
def redirectstderrout(runtype=None, uniquenum=None, uppernum=None):
    ...
    mystderrname = os.getcwd() + "/python_u_%d_%d_%d.stderr.out" % (
        runtype,
        uniquenum,
        uppernum,
    )
    mystdoutname = os.getcwd() + "/python_u_%d_%d_%d.stdout.out" % (
        runtype,
        uniquenum,
        uppernum,
    )
...
def runglobalsetup(argv=None):
    ...
    if runtype == 2 or runtype == 3 or runtype == 4:
        if len(sys.argv[4:]) > 0 and argv[3] != "plot":
            ...
            uppernum = int(argv[4])
            print(
                "runtype=%d has runnumber=%d uppernum=%d"
                % (runtype, runnumber, uppernum)
            )
            sys.stdout.flush()
            ...
            print("Doing plot type of run")
            sys.stdout.flush()
        else:
            print(
                "runtype=%d should have runnumber and uppernum but doesn't!" % (runtype)
            )
            sys.stdout.flush()
            exit
    #
...
from harm_pythontools.streamlines import (
    streamplot,
)  # streamlines is from harm_pythontools, not actually a python package
...
def printusage():
    ...
    # below for unix
    memoryusage = memory()
    print("memoryusage=%g" % (memoryusage))
    sys.stdout.flush()
    #
...
def tensordot01(uu, dxdxp):
    #
    result = (
        (uu[:, None, None, None] * dxdxp[:, :, None, None, None])
        .sum(axis=1)
        .reshape(4, nx, ny, nz)
    )
    #
...
def tensordot00(uu, dxdxp):
    ...
    result = (
        (uu[:, None, None, None] * dxdxptrans[:, :, None, None, None])
        .sum(axis=1)
        .reshape(4, nx, ny, nz)
    )
    return result
...
def intangle_foravg2d(
    ...
    result = intangle(
        ...
        hoverr=hoverr,
        thetamid=thetamid,
        minbsqorho=minbsqorho,
        maxbsqorho=maxbsqorho,
        inflowonly=inflowonly,
...
def elinfcalc(a):
    ...
    #
    if a < 0.9999999:
        einf = (1.0 - 2.0 / risco + a / (risco) ** (3.0 / 2.0)) / (
            1.0 - 3.0 / risco + 2.0 * a / (risco) ** (3.0 / 2.0)
        ) ** (1.0 / 2.0)
        # print( "einf=%g" % (einf) )
        linf = (np.sqrt(risco) * (risco**2.0 - 2.0 * a * np.sqrt(risco) + a**2)) / (
            risco
            * (risco**2.0 - 3.0 * risco + 2.0 * a * np.sqrt(risco)) ** (1.0 / 2.0)
        )
        # print( "linf=%g" % (linf) )
    else:
        if risco < 2.0:
...
def Risco(a):
    Z1 = 1 + (1.0 - a**2) ** (1.0 / 3.0) * (
        (1.0 + a) ** (1.0 / 3.0) + (1.0 - a) ** (1.0 / 3.0)
    )
    Z2 = (3 * a**2 + Z1**2) ** (1.0 / 2.0)
...
def odot(a, b):
    ...
    outer_product = np.zeros(
        np.concatenate((np.array((4, 4)), amax(a[0].shape, b[0].shape))),
        dtype=np.float32,
        order="F",
    )
...
def fix_hp(th, ph):
    ...
    ph[np.logical_and(np.logical_or(th < 0.0, th >= M_PI), ph <= M_PI)] = (
        ph[np.logical_and(np.logical_or(th < 0.0, th >= M_PI), ph <= M_PI)] + M_PI
    )
    ph[np.logical_and(np.logical_or(th < 0.0, th >= M_PI), ph > M_PI)] = (
        ph[np.logical_and(np.logical_or(th < 0.0, th >= M_PI), ph > M_PI)] - M_PI
    )
...
def areahor():
    ...
    print("area=%g" % (area))
    areaup = np.sum(
        unit[ihor, 0 : ny / 2, :] * gdet[ihor + 1, 0 : ny / 2, :] * _dx2 * _dx3
    )
    print("areaup=%g" % (areaup))
...
def horfluxcalc(
    ...
    if takecumsum == 0:
        ...
        if ivalue == None and jvalue == None:
            ...
            if takeextreme == 1:
                bigj = np.where(
                    np.fabs(fabs[ivalue, :]) == np.max(np.fabs(fabs[ivalue, :]))
                )[0]
                # print("bigj=%d" % (bigj) )
...
def set_transV2Vmetric(Vmetric=None, b0=0.0):
    ...
    transV2Vmetric[0, 0] = 1.0 + gv3[0, 0] * 0.0
    transV2Vmetric[0, 1] = 0.0 + gv3[0, 0] * 0.0
    transV2Vmetric[0, 2] = 0.0 + gv3[0, 0] * 0.0
    transV2Vmetric[0, 3] = 0.0 + gv3[0, 0] * 0.0
    transV2Vmetric[1, 0] = 0.0 + gv3[0, 0] * 0.0
    transV2Vmetric[1, 1] = 1.0 + gv3[0, 0] * 0.0
    transV2Vmetric[1, 2] = 0.0 + gv3[0, 0] * 0.0
    transV2Vmetric[1, 3] = 0.0 + gv3[0, 0] * 0.0
    transV2Vmetric[2, 0] = 0.0 + gv3[0, 0] * 0.0
    transV2Vmetric[2, 1] = 0.0 + gv3[0, 0] * 0.0
    transV2Vmetric[2, 2] = pow(
        pow(cos(h) * sin(b0) - 1.0 * cos(b0) * cos(ph) * sin(h), 2.0)
        + pow(sin(h), 2.0) * pow(sin(ph), 2.0),
        -0.5,
    ) * (-1.0 * cos(h) * cos(ph) * sin(b0) + cos(b0) * sin(h))
    transV2Vmetric[2, 3] = (
        pow(
            pow(cos(h) * sin(b0) - 1.0 * cos(b0) * cos(ph) * sin(h), 2.0)
            + pow(sin(h), 2.0) * pow(sin(ph), 2.0),
    ...
    transV2Vmetric[3, 0] = 0.0 + gv3[0, 0] * 0.0
    transV2Vmetric[3, 1] = 0.0 + gv3[0, 0] * 0.0
    transV2Vmetric[3, 2] = (
        -1.0
        * pow(
            pow(cos(h) * sin(b0) - 1.0 * cos(b0) * cos(ph) * sin(h), 2.0)
    ...
    if 0 == 1:  # don't need, so don't waste memory
        ...
        transVmetric2V[0, 0] = 1.0 + gv3[0, 0] * 0.0
        transVmetric2V[0, 1] = 0.0 + gv3[0, 0] * 0.0
        transVmetric2V[0, 2] = 0.0 + gv3[0, 0] * 0.0
        transVmetric2V[0, 3] = 0.0 + gv3[0, 0] * 0.0
        transVmetric2V[1, 0] = 0.0 + gv3[0, 0] * 0.0
        transVmetric2V[1, 1] = 1.0 + gv3[0, 0] * 0.0
        transVmetric2V[1, 2] = 0.0 + gv3[0, 0] * 0.0
        transVmetric2V[1, 3] = 0.0 + gv3[0, 0] * 0.0
        transVmetric2V[2, 0] = 0.0 + gv3[0, 0] * 0.0
        transVmetric2V[2, 1] = 0.0 + gv3[0, 0] * 0.0
        transVmetric2V[2, 2] = (
            pow(
                pow(cos(h) * cos(ph) * sin(b0) - 1.0 * cos(b0) * sin(h), 2.0)
                + pow(sin(b0), 2.0) * pow(sin(ph), 2.0),
                -1.0,
            )
            * pow(
                pow(cos(h) * sin(b0) - 1.0 * cos(b0) * cos(ph) * sin(h), 2.0)
        ...
        transVmetric2V[2, 3] = -1.0 * sin(b0) * sin(ph)
        transVmetric2V[3, 0] = 0.0 + gv3[0, 0] * 0.0
        transVmetric2V[3, 1] = 0.0 + gv3[0, 0] * 0.0
        transVmetric2V[3, 2] = (
            csc(h)
            * pow(
                pow(cos(h) * cos(ph) * sin(b0) - 1.0 * cos(b0) * sin(h), 2.0)
        ...
        transVmetric2V[3, 3] = cos(b0) - 1.0 * cos(ph) * cot(h) * sin(b0)
        #
...
def grid3d(
    dumpname, use2d=False, doface=False, usethetarot0=False
):  # read grid dump file: header and body
    #
    #
    #
    ...
    #
    print("realdumpname=%s" % (realdumpname))
    sys.stdout.flush()
    #
    ...
    print("Done grid3d!")
    sys.stdout.flush()
    return nx, ny, nz, _startx1, _startx2, _startx3, _dx1, _dx2, _dx3, a, Rin, Rout, gam


def grid3d_load(
    dumpname=None, use2d=False, doface=False, loadsimple=False
):  # read grid dump file: header and body
    ...
    if use2d:
        ...
        lnz = nz
    #
    print("Done reading grid header")
    sys.stdout.flush()
    #
    ncols = 126
    if dumpname.endswith(".bin"):
        print("Start reading grid as binary with lnz=%d" % (lnz))
        sys.stdout.flush()
        body = np.fromfile(gin, dtype=np.float64, count=ncols * nx * ny * lnz)
        gd = body.view().reshape((-1, nx, ny, lnz), order="F")
        gin.close()
        print("Done reading grid as binary with lnz=%d" % (lnz))
        sys.stdout.flush()
    else:
        print("Start reading grid as text with lnz=%d" % (lnz))
        sys.stdout.flush()
        gin.close()
        gd = (
            np.loadtxt("dumps/" + dumpname, dtype=np.float64, skiprows=1, unpack=True)
            .view()
            .reshape((126, nx, ny, lnz), order="F")
        )
        print("End reading grid as text with lnz=%d" % (lnz))
        sys.stdout.flush()
    gd = myfloat(gd)
    gc.collect()
    #
...
def gridcellverts():
    ...
    rf[1:nx, 0:ny, 0:lnz] = (
        r[1:nx] * r[0 : nx - 1]
    ) ** 0.5  # - 0.125*(dxdxp[1,1,1:nx]/r[1:nx]-dxdxp[1,1,0:nx-1]/r[0:nx-1])*_dx1
    # extend in theta
    ...
    hf[0:nx, 1:ny, 0:lnz] = 0.5 * (
        h[:, 1:ny] + h[:, 0 : ny - 1]
    )  # - 0.125*(dxdxp[2,2,:,1:ny]-dxdxp[2,2,:,0:ny-1])*_dx2
    hf[1 : nx - 1, 1:ny, 0:lnz] = 0.5 * (
        hf[0 : nx - 2, 1:ny, 0:lnz] + hf[1 : nx - 1, 1:ny, 0:lnz]
    )
    ...
    tif = np.arange(0, (nx + 1) * (ny + 1) * (lnz + 1)).reshape(
        (nx + 1, ny + 1, lnz + 1), order="F"
    )
    tjf = np.arange(0, (nx + 1) * (ny + 1) * (lnz + 1)).reshape(
        (nx + 1, ny + 1, lnz + 1), order="F"
    )
    tkf = np.arange(0, (nx + 1) * (ny + 1) * (lnz + 1)).reshape(
        (nx + 1, ny + 1, lnz + 1), order="F"
    )
    tif %= nx + 1
...
def rfd(fieldlinefilename, **kwargs):
    ...
    if nx == nxgdump and ny == nygdump and nz == nzgdump and THETAROT == THETAROTgdump:
        print("fieldline file has same nx,ny,nz,THETAROT as gdump file")
        sys.stdout.flush()
    else:
        print(
            "fieldline file has different nx,ny,nz,THETAROT as gdump file, so reading correct gdump file"
        )
        sys.stdout.flush()
        if THETAROT == 0.0:
    ...
    if 0:
        ...
        d = np.memmap(
            fin, dtype="float32", mode="c", shape=(numcolumns, nx, ny, nz), order="F"
        )
    #
    else:
        # old way:
    ...
    uu = d[
        4:8, :, :, :
    ]  # again, note uu[i] are 3-velocities (as read from the fieldline file)
    ...
    if whichpoledeath == 2:
        if np.fabs(THETAROT > 0.0):
            ...
            # now back to uu
            qsq = (
                gv3[1, 1] * urel[1] * urel[1]
                + gv3[2, 2] * urel[2] * urel[2]
                + gv3[3, 3] * urel[3] * urel[3]
                + 2.0
                * (
                    gv3[1, 2] * urel[1] * urel[2]
                    + gv3[1, 3] * urel[1] * urel[3]
                    + gv3[2, 3] * urel[2] * urel[3]
                )
            )
    ...
    print("numcolumnshere: %d" % (numcolumns))
    sys.stdout.flush()
    #
    gotgdetB = 0
    if d.shape[0] >= 14 and numcolumns == 11 + 3:
        gotgdetB = 1
        print("Getting gdetB: dshape0=%d" % (d.shape[0]))
        sys.stdout.flush()
    ...
    if numcolumns == 16:
        ...
        maxErf = np.max(Erf)
        minErf = np.min(Erf)
        print("maxErf=%g minErf=%g" % (maxErf, minErf))
        sys.stdout.flush()
        #
    else:
        Erf = rho * 0 + 1e-30
    ...
    if use2dglobal == True and (DEBUGTHETAROT or np.fabs(THETAROT - 0.0) > 1e-13):
        #
        print(
            "rfd(before rfdtransform) time elapsed: %d"
        ...
        if os.path.exists(fnamenpz):
            ...
            print("THETAROT=%21.15g for rfdtransform" % (THETAROT))
            sys.stdout.flush()
            ...
            #
            print(
                "rfd(after rfdtransform) time elapsed: %d"
                % (datetime.now() - start_time).seconds
            )
            sys.stdout.flush()
    else:
        print("No rdtrans for file=%s with THETAROT=%g" % (fname, THETAROT))
        sys.stdout.flush()
    #
    #
    #############################################################################################
    ...
    #
    print(
        "rfd(after rfdprocess) time elapsed: %d" % (datetime.now() - start_time).seconds
    )
    sys.stdout.flush()
...
def rfdheader(fin=None):
    ...
    if numheaderitems == 53:
        ...
        WHICHENTROPYEVOLVE = (
            1  # EVOLVESIMPLEENTROPY with DODISS=0 and EVOLVEFULLENTROPY with off
        )
...
def rfdprocess(gotgdetB=0):
    ...
    if "r" in globals() and r.shape[2] != nz:
        ...
        a_phi = phiwedge / (2.0 * nz) + np.linspace(0, phiwedge, num=nz, endpoint=False)
        ...
        print("r in globals has shape2 of nz")
        sys.stdout.flush()
    #
    # other stuff
    entropy = (gam - 1.0) * ugclean / rho ** (gam)
...
def rfdtransform(gotgdetB=0):
    ...
    start_time = datetime.now()
    print(
        "rfdtransform(start) time elapsed: %d" % (datetime.now() - start_time).seconds
    )
    sys.stdout.flush()
    #
    # modified globals
    ...
    print("pre-r2d shape (nx=%d ny=%d)" % (nx, ny))
    sys.stdout.flush()
    print(r[:, :, 0].shape)
    sys.stdout.flush()
    # .view().reshape(-1)
    ...
    #
    print(
        "rfdtransform(done assign Vmetric) time elapsed: %d"
        % (datetime.now() - start_time).seconds
    )
    sys.stdout.flush()
    # DEBUG:
    ...
    sys.stdout.flush()
    print(
        "rfdtransform(done assign Vorig) time elapsed: %d"
        % (datetime.now() - start_time).seconds
    )
    sys.stdout.flush()
    #
    ...
    if 0 == 1:
        ...
        if gotgdetB == 1:
            (
                ...
                gdetB[2],
                gdetB[3],
            ) = reinterp3dspc_opt_all(Vorig, Vmetric, rho, ug, uu, B, gdetB=gdetB)
        else:
            (
                rho,
                ug,
                ...
                B[2],
                B[3],
            ) = reinterp3dspc_opt_all(Vorig, Vmetric, rho, ug, uu, B)
    #
    print("reinterp3dspc END\n")
    sys.stdout.flush()
    print(
        "rfdtransform(done assign interpolated prims) time elapsed: %d"
        % (datetime.now() - start_time).seconds
    )
    sys.stdout.flush()
    #
    ...
    utest1old[2] = 0.0
    utest1old[3] = 0.0
    utest1new = np.tensordot(
        utest1old, transV2Vmetric[:, :, 60, ny / 2, nz / 4], axes=[0, 1]
    )
    print("utest1new")
    sys.stdout.flush()
    print(utest1new)
    ...
    print(
        "rfdtransform(done get transV2Vmetric) time elapsed: %d"
        % (datetime.now() - start_time).seconds
    )
    sys.stdout.flush()
    #
    ...
    if gotgdetB == 1:
        ...
        if 0 == 1:
            ...
            gdetBnew1 = (
                gdetB  # assume already applied dxdxp at same r,theta,phi location as uu
            )
        #
        gdetBnew2 = tensordot01(
            gdetBnew1, transV2Vmetric
        )  # ,axes=([0],[1])) # now u^\nu[Vmetric]
        gdetBnew3 = tensordot00(
            gdetBnew2, idxdxp
        )  # ,axes=([0],[0])) # now u^\nu[Xmetric]
    ...
    sys.stdout.flush()
    print(
        "rfdtransform(done tensordots) time elapsed: %d"
        % (datetime.now() - start_time).seconds
    )
    sys.stdout.flush()
...
def rddims(gotrad):
    ...
    if gotrad == 1:
        ...
        rhoedd = (
            Mdotedd
            / CCCTRUE
            * (GGG * MPERSUN * MSUN / CCCTRUE**2)
            / (GGG * MPERSUN * MSUN / CCCTRUE**2) ** 3
        )
        ...
        #
        print(
            "CCCTRUE=%g ENBAR=%g TBAR=%g Ledd=%g Mdotedd=%g einf=%g linf=%g uedd=%g bedd=%g"
            % (CCCTRUE, ENBAR, TBAR, Ledd, Mdotedd, einf, linf, uedd, bedd)
        )
        sys.stdout.flush()
        print("CCCTRUE=%g ENBAR=%g TBAR=%g" % (CCCTRUE, ENBAR, TBAR))
        sys.stdout.flush()
    else:
        GGG = 1
        CCCTRUE = 1
...
def getkappas(gotrad):
    ...
    if gotrad == 0:
        ...
        pg = (
            gam - 1.0
        ) * ug  # clean # use clean to keep pg low and Tgas will have floor like below  # no, need to use what was in simulation to be consistent with simulation's idea of what optical depth was
...
def cvel():
    ...
    hoverr3dtoplot, thetamid3dtoplot = horcalc(
        hortype=1, which1=diskcondition1, which2=diskcondition2, denfactor=rholab
    )
    ...
    (
        taurad1integrated,
        taurad1flipintegrated,
        taurad2integrated,
        taurad2flipintegrated,
        tauradintegrated,
        tauradeff1integrated,
        tauradeff1flipintegrated,
        tauradeff2integrated,
        tauradeff2flipintegrated,
        tauradeffintegrated,
    ) = compute_taurad()
...
def getrhouclean(rho, ug, uu):
    ...
    if len3rho == 1 and nz > 1:
        ...
        print("rho wasn't nz in size, so assume 2D avg with only nz=1 effectively")
        sys.stdout.flush()
    #
    #
    ...
    #
    rinterp = (
        (faker - 9.0) * (1.0 - 0.0) / (0.0 - 9.0)
    )  # gives 0 for use near 9   gives 1 for use near 0
    rinterp[rinterp > 1.0] = 1.0
    rinterp[rinterp < 0.0] = 0.0
    #
    condmaxbsqorhorhs = rinterp * maxbsqorhonear + (1.0 - rinterp) * maxbsqorhofar
    condmaxbsqorho = (
        bsq / rho < condmaxbsqorhorhs
    )  # used as spatial conditional to replace single value of maxbsqorho
    #
    print("t=%g" % (t))
    print("r")
    print(r[:, ny // 2, 0])
    ...
    if douuclean == 1:
        ...
        omegafpara = (0.25 * np.sin(hbz) ** 2 * (1 + np.log(1 + np.cos(hbz)))) / (
            4.0 * np.log(2.0)
            + np.sin(hbz) ** 2
            + (np.sin(hbz) ** 2 - 2 * (1.0 + np.cos(hbz))) * np.log(1.0 + np.cos(hbz))
        )
        ...
        print("numcond=%d numcond2=%d" % (numcond, numcond2))
        sys.stdout.flush()
        #
        maxiter = 5
        for iter in np.arange(0, maxiter):
            ...
            if 1 == 0:
                ...
                print(
                    "iter=%d numcondbaduu0=%d pickuu0sqnew=%g\n",
                    iter,
                    numcondbaduu0,
                    uu0sqnew[condbaduu0],
                )
                ...
                uuclean[1][condbaduu0 == 0] = (
                    vunew[1][condbaduu0 == 0] * uu0new[condbaduu0 == 0]
                )
                uuclean[2][condbaduu0 == 0] = (
                    vunew[2][condbaduu0 == 0] * uu0new[condbaduu0 == 0]
                )
                uuclean[3][condbaduu0 == 0] = (
                    vunew[3][condbaduu0 == 0] * uu0new[condbaduu0 == 0]
                )
            else:
                ...
                condpickuu0 = np.fabs(uu0newb - pickeduu0new) < np.fabs(
                    uu0newa - pickeduu0new
                )
                ...
                #
                print(
                    "iter=%d numcondbaduu0=%d sumcondpickuu0=%d uu0newabad=%g uu0newbbad=%g\n",
                    iter,
                    numcondbaduu0,
                    sumcondpickuu0,
                    uu0newa[condbaduu0],
        ...
        print("chosenomegaf=%g omegarattest=%g ", chosenomegaf, omegarattest)
        sys.stdout.flush()
    #
    return (
        rhoclean,
...
def compute_resires(hoverrwhich=None):
    ...
    mydH = (
        r * dxdxp[2][2] * _dx2
    )  # GODMARK: inaccurate a bit as \theta component because dxdxp[1][2] and dxdxp[2][1] are non-zero.  So say so in paper.
    ...
    omega = (
        1.0 + r * 0.0
    )  # now no longer using OMEGAFIX (i.e. OMEGAFIX=0 should be set)
    #
    #####################################
    #
...
def fieldcalcface(gdetB1=None):
    ...
    aphi[:, ny // 2 + 1 : ny] = (-daphi[:, ::-1].cumsum(axis=1))[:, ::-1][
        :, ny // 2 + 1 : ny
    ]
    return aphi
...
def compute_taurad(domergeangles=True, radiussettau1zero=80):
    ...
    ############# taurad1
    taurad1[
        r[:, 0, 0] > radiussettau1zero, :, :
    ] = 0  # to get rid of parts of flow that aren't in steady-state and wouldn't have contributed
...
def get2davg(
    ...
    if usedefault == 0 and (
        whichgroups < 0
        or whichgroupe < 0
        or whichgroups >= whichgroupe
        or itemspergroup <= 0
    ):
        print(
            "get2davg: whichgroups = %d, whichgroupe = %d, itemspergroup = %d not allowed"
    ...
    #
    print("gdet2davg(): checking for fname=%s" % (fname))
    sys.stdout.flush()
    #
    ...
    for i, g in enumerate(myrange):
        ...
        if domerge == True:
            ...
            if (tftry != 0 and tstry != 0) or (tftry != 0):
                if (
                    (tstry > avgfti and tstry < avgftf)
                    or ((tftry > avgfti and tftry < avgftf))
                    or ((tstry < avgfti and tftry > avgftf))
                ):
                    ...
                    n2avg += 1
                    print(
                        "USING: During merge: ts=%g tf=%g tstry=%g tftry=%g n2avg=%d"
                        % (ts, tf, tstry, tftry, n2avg)
                    )
                    sys.stdout.flush()
                    didntuse = 0
                    #
            ...
            if didntuse == 1:
                print(
                    "NOTUSING: During merge: tstry=%g tftry=%g n2avg=%d avgfti=%g avgftf=%g"
                    % (tstry, tftry, n2avg, avgfti, avgftf)
                )
                sys.stdout.flush()
        else:
            ...
            n2avg += 1
            print(
                "During NONmerge: ts=%g tf=%g tstry=%g tftry=%g n2avg=%d"
                % (ts, tf, tstry, tftry, n2avg)
            )
            sys.stdout.flush()
    ...
    avgtot[0, 2, 0] = nitems
    print(
        "Final avg_ts=%g avg_te=%g nitems=%d localdenom=%g"
        % (avgtot[0, 0, 0], avgtot[0, 1, 0], avgtot[0, 2, 0], localdenom)
    )
    sys.stdout.flush()
    # get the average
    if n2avg == 0:
    ...
    avgtot[1:] /= localdenom
    #
    print("avgtot[3] for avg_bsq in merge")
    sys.stdout.flush()
    print(avgtot[3])
    sys.stdout.flush()
    #
    ...
    if numrange > 1:
        print("Saving data to file: n2avg=%d ..." % (n2avg))
        sys.stdout.flush()
...
def assignavg2dvars(avgmem):
    ...
    # quantities
    # 4
    avg_rho = avgmem[i, :, :, None]
    i += 1  # i=1
    avg_ug = avgmem[i, :, :, None]
    i += 1  # i=2
    avg_urad = avgmem[i, :, :, None]
    i += 1  # i=3
    avg_bsq = avgmem[i, :, :, None]
    i += 1  # i=4
    avg_unb = avgmem[i, :, :, None]
    i += 1
    # 4*4=16
    n = 4
    avg_uu = avgmem[i : i + n, :, :, None]
    i += n
    avg_bu = avgmem[i : i + n, :, :, None]
    i += n
    avg_ud = avgmem[i : i + n, :, :, None]
    i += n
    avg_bd = avgmem[i : i + n, :, :, None]
    i += n
    # 4*4=16
    n = 4
    avg_absuu = avgmem[i : i + n, :, :, None]
    i += n
    avg_absbu = avgmem[i : i + n, :, :, None]
    i += n
    avg_absud = avgmem[i : i + n, :, :, None]
    i += n
    avg_absbd = avgmem[i : i + n, :, :, None]
    i += n
    # cell-centered magnetic field components
    # 3*2=6
    n = 3
    avg_B = avgmem[i : i + n, :, :, None]
    i += n
    avg_gdetB = avgmem[i : i + n, :, :, None]
    i += n
    # 3*2=6
    n = 3
    avg_absB = avgmem[i : i + n, :, :, None]
    i += n
    avg_absgdetB = avgmem[i : i + n, :, :, None]
    i += n
    # 4
    avg_omegaf2 = avgmem[i, :, :, None]
    i += 1
    avg_omegaf2b = avgmem[i, :, :, None]
    i += 1
    avg_omegaf1 = avgmem[i, :, :, None]
    i += 1
    avg_omegaf1b = avgmem[i, :, :, None]
    i += 1
    #
    # 4
    avg_absomegaf2 = avgmem[i, :, :, None]
    i += 1
    avg_absomegaf2b = avgmem[i, :, :, None]
    i += 1
    avg_absomegaf1 = avgmem[i, :, :, None]
    i += 1
    avg_absomegaf1b = avgmem[i, :, :, None]
    i += 1
    #
    # 6*4=24
    n = 4
    avg_rhouu = avgmem[i : i + n, :, :, None]
    i += n
    avg_rhobu = avgmem[i : i + n, :, :, None]
    i += n
    avg_rhoud = avgmem[i : i + n, :, :, None]
    i += n
    avg_rhobd = avgmem[i : i + n, :, :, None]
    i += n
    avg_uguu = avgmem[i : i + n, :, :, None]
    i += n
    avg_ugud = avgmem[i : i + n, :, :, None]
    i += n
    #
    # 1*4=4
    n = 4
    avg_absrhouu = avgmem[i : i + n, :, :, None]
    i += n
    #
    # 2*16=32
    n = 16
    # energy fluxes and faraday
    avg_Tud = avgmem[i : i + n, :, :, None].reshape((4, 4, nx, ny, 1))
    i += n
    avg_fdd = avgmem[i : i + n, :, :, None].reshape((4, 4, nx, ny, 1))
    i += n
    # 1*16=16
    n = 16
    # faraday
    avg_absfdd = avgmem[i : i + n, :, :, None].reshape((4, 4, nx, ny, 1))
    i += n
    # 5*16=80
    # part1: rho u^m u_l
    avg_rhouuud = avgmem[i : i + n, :, :, None].reshape((4, 4, nx, ny, 1))
    i += n
    # part2: u u^m u_l
    avg_uguuud = avgmem[i : i + n, :, :, None].reshape((4, 4, nx, ny, 1))
    i += n
    # part3: b^2 u^m u_l
    avg_bsquuud = avgmem[i : i + n, :, :, None].reshape((4, 4, nx, ny, 1))
    i += n
    # part6: b^m b_l
    avg_bubd = avgmem[i : i + n, :, :, None].reshape((4, 4, nx, ny, 1))
    i += n
    # u^m u_l
    # print( "i = %d, avgmem.shape[0] = %d " % (i, avgmem.shape[0]) )
    # sys.stdout.flush()
    avg_uuud = avgmem[i : i + n, :, :, None].reshape((4, 4, nx, ny, 1))
    i += n
    # 2*16=32
    n = 16
    # EM/MA
    avg_TudEM = avgmem[i : i + n, :, :, None].reshape((4, 4, nx, ny, 1))
    i += n
    avg_TudMA = avgmem[i : i + n, :, :, None].reshape((4, 4, nx, ny, 1))
    i += n
    # 2*16=32
    n = 16
    # P/IE
    avg_TudPA = avgmem[i : i + n, :, :, None].reshape((4, 4, nx, ny, 1))
    i += n
    avg_TudEN = avgmem[i : i + n, :, :, None].reshape((4, 4, nx, ny, 1))
    i += n
    avg_TudRAD = avgmem[i : i + n, :, :, None].reshape((4, 4, nx, ny, 1))
    i += n
    # 3
    # mu,sigma
    n = 1
    avg_mu = avgmem[i, :, :, None]
    i += n
    avg_sigma = avgmem[i, :, :, None]
    i += n
    avg_bsqorho = avgmem[i, :, :, None]
    i += n
    # 1
    n = 1
    avg_psisq = avgmem[i, :, :, None]
    i += n
    #
    avg_KAPPAUSER = avgmem[i, :, :, None]
    i += 1  # i=1
    avg_KAPPAESUSER = avgmem[i, :, :, None]
    i += 1  # i=1
    avg_tauradintegrated = avgmem[i, :, :, None]
    i += 1  # i=1
    avg_tauradeffintegrated = avgmem[i, :, :, None]
    i += 1  # i=1
    #
    avg_vpot = avgmem[i, :, :, None]
    i += 1  # i=1
    avg_vpot2 = avgmem[i, :, :, None]
    i += 1  # i=1
    #
...
def get2davgone(whichgroup=-1, itemspergroup=20):
    ...
    if whichgroup < 0 or itemspergroup <= 0:
        print(
            "get2davgone: whichgroup = %d, itemspergroup = %d not allowed"
            % (whichgroup, itemspergroup)
        )
        sys.stdout.flush()
        return None
    #
    ...
    for fldindex, fldname in enumerate(flist):
        ...
        n = 16
        # faraday
        avg_absfdd += (np.fabs(fdd)).sum(-1)[:, :, :, :, None] * localdt[
            itert
        ]  # take absolute value since oscillate around 0 near equator and would cancel out and give noise in fdd/fdd type calculations, such as for omegaf
        #
        #
        ...
        avg_bsqorho += (bsq / rhounclean).sum(-1)[:, :, None] * localdt[
            itert
        ]  # keep as unclean since want to know what bsqorho is
        #
        #
        ...
        avg_tauradeffintegrated += (
            tauradeffintegrated.sum(-1)[:, :, None] * localdt[itert]
        )
        # 1
        n = 1
        aphi = fieldcalcface()
        ...
        # 1 and 1
        n = 1
        avg_vpot += (scaletofullwedge(nz * _dx3 * fieldcalc(gdetB1=gdetB[0]))).sum(-1)[
            :, :, None
        ] * localdt[itert]
        avg_vpot2 += (scaletofullwedge((nz * (_dx3 * aphi) ** 2)) ** 0.5).sum(-1)[
            :, :, None
        ] * localdt[itert]
        #
        # iterate
    ...
    denom = localdt.sum()
    #
    print("denom=%g and localdt[0]=%g" % (denom, localdt[0]))
    sys.stdout.flush()
    #
    print("avg_bsq when forming")
    sys.stdout.flush()
    print(avg_bsq)
    sys.stdout.flush()
    #
    ...
    #
    print("avg_bsq when setting avgmem")
    sys.stdout.flush()
    print(avgmem[3])
    sys.stdout.flush()
    #
...
def getnqtyavg():
    ...
    value = (
        value + 16 + 4
    )  # for TudRAD and KAPPAUSER and KAPPAESUSER and tauradintegrated and tauradeffintegrated
...
def Tcalcud(maxbsqorho=None, which=None):
    ...
    for kapa in np.arange(4):
        for nu in np.arange(4):
            if kapa == nu:
                ...
                delta = 0
            TudEM[kapa, nu] = (
                bsq * uu[kapa] * ud[nu] + 0.5 * bsq * delta - bu[kapa] * bd[nu]
            )
    ...
    #
    #
    print("TudEM[1,0,5,0,0]=%g" % (TudEM[1, 0, 5, 0, 0]))
    sys.stdout.flush()
    print("bsq[5,0,0]=%g" % (bsq[5, 0, 0]))
    sys.stdout.flush()
    print("uu[1,5,0,0]=%g" % (uu[1, 5, 0, 0]))
    sys.stdout.flush()
    print("ud[0,5,0,0]=%g" % (ud[0, 5, 0, 0]))
    sys.stdout.flush()
    print("bu[1,5,0,0]=%g" % (bu[1, 5, 0, 0]))
    sys.stdout.flush()
    print("bd[0,5,0,0]=%g" % (bd[0, 5, 0, 0]))
    sys.stdout.flush()
    #
    print("uu[0,5,0,0]=%g" % (uu[0, 5, 0, 0]))
    sys.stdout.flush()
    print("B[0,5,0,0]=%g" % (B[0, 5, 0, 0]))
    sys.stdout.flush()
    print("uu[1,5,0,0]=%g" % (uu[1, 5, 0, 0]))
    sys.stdout.flush()
    print("B[1,5,0,0]=%g" % (B[1, 5, 0, 0]))
    sys.stdout.flush()
    print("uu[2,5,0,0]=%g" % (uu[2, 5, 0, 0]))
    sys.stdout.flush()
    print("B[2,5,0,0]=%g" % (B[2, 5, 0, 0]))
    sys.stdout.flush()
    print("uu[3,5,0,0]=%g" % (uu[3, 5, 0, 0]))
    sys.stdout.flush()
    print("B[3,5,0,0]=%g" % (B[3, 5, 0, 0]))
    sys.stdout.flush()
    print(
        "udotB[5,0,0]=%g"
        % (
            ud[0, 5, 0, 0] * B[0, 5, 0, 0]
            + ud[1, 5, 0, 0] * B[1, 5, 0, 0]
            + ud[2, 5, 0, 0] * B[2, 5, 0, 0]
            + ud[3, 5, 0, 0] * B[3, 5, 0, 0]
        )
    )
    sys.stdout.flush()
...
def faraday():
    ...
    omegaf1b = v3nonhat - B3nonhat * (v1hat * B1hat + v2hat * B2hat) / (
        B1hat**2 + B2hat**2
    )
...
def checkiffullavgexists():
    #
    fname = "avg2d.npy"
    print("checkiffullavgexists(): checking for fname=%s" % (fname))
    sys.stdout.flush()
    #
    if os.path.isfile(fname):
...
def plc(
    myvar, xcoord=None, ycoord=None, ax=None, picker=False, **kwargs
):  # needs debugging for 2.7.18
    #
    #
    #
    ...
    if xcoord == None or ycoord == None:
        if nc == 15:  # poor man's way to choose
            res = ax.contour(
                myvar[:, :, 0].transpose(), levels=levels, picker=picker, **kwargs
            )
        else:
...
def plot_slice(fnumber, phi, cap, floor, IC=True, stress=True):
    ...
    if 1 == 1:
        ...
        diskcondition = (
            diskcondition * (bsq / rho < 1.0) * (np.fabs(h - np.pi * 0.5) < 0.1)
        )
    ...
    if IC:
        ...
        myx = (
            r[nxin:nxout, ny / 2, :]
            * np.sin(h[nxin:nxout, ny / 2, :])
            * np.cos(ph[nxin:nxout, ny / 2, :])
        )
        myy = (
            r[nxin:nxout, ny / 2, :]
            * np.sin(h[nxin:nxout, ny / 2, :])
            * np.sin(ph[nxin:nxout, ny / 2, :])
        )
    ...
    if stress:
        ...
        if avgexists == 1:
            ...
            numRey = jabs(
                rho * (uu[1]) * np.sqrt(gv3[1, 1]) * (ud[3]) * np.sqrt(gn3[3, 3])
            )
        arey = numRey / denMR
    ...
    ax.pcolor(
        myx,
        myy,
        myfun[nxin:nxout, ny / 2, :],
        norm=[None, MidpointNormalize(midpoint=0)][ax >= 1],
    )  # try pcolormesh - faster
    plc(
        myfun[nxin:nxout, ny / 2, :],
        xcoord=myx,
        ycoord=myy,
...
def reinterp3dspc(Vorig, Vmetric, vartointerp):
    ...
    if 1 == 0:
        ...
        varinterpolatedfun = sp.interpolate.LinearNDInterpolator(
            pts, vartointerp.ravel(), fill_value=0.0
        )
        ptsnew = np.array(
            (Vmetric[1].ravel(), Vmetric[2].ravel(), Vmetric[3].ravel())
        ).T
        varinterpolated = varinterpolatedfun(
            ptsnew
        )  # ((Vmetric[1], Vmetric[2], Vmetric[3]))
    else:
        pts = np.array((Vorig[1].ravel(), Vorig[2].ravel(), Vorig[3].ravel())).T
        ptsnew = np.array(
            (
                Vmetric[1, 20, 15, 20].ravel(),
                Vmetric[2, 20, 15, 20].ravel(),
                Vmetric[3, 20, 15, 20].ravel(),
            )
        ).T
        print("ptsnew")
        sys.stdout.flush()
        print(ptsnew)
        sys.stdout.flush()
        varinterpolatedfun1 = sp.interpolate.LinearNDInterpolator(
            pts, Vorig[1].ravel(), fill_value=0.0
        )
        print("duck1")
        sys.stdout.flush()
        result1 = varinterpolatedfun1(ptsnew)  # ((Vmetric[1], Vmetric[2], Vmetric[3]))
        print(
            "results: Vmetric: %g %g %g  :: Vorig1: %g\n"
            % (
                Vmetric[1, 20, 15, 20],
                Vmetric[2, 20, 15, 20],
                Vmetric[3, 20, 15, 20],
        ...
        varinterpolatedfun2 = sp.interpolate.LinearNDInterpolator(
            pts, Vorig[2].ravel(), fill_value=0.0
        )
        print("duck2")
        sys.stdout.flush()
        result2 = varinterpolatedfun2(ptsnew)  # ((Vmetric[1], Vmetric[2], Vmetric[3]))
        print(
            "results: Vmetric: %g %g %g  :: Vorig2: %g\n"
            % (
                Vmetric[1, 20, 15, 20],
                Vmetric[2, 20, 15, 20],
                Vmetric[3, 20, 15, 20],
        ...
        varinterpolatedfun3 = sp.interpolate.LinearNDInterpolator(
            pts, Vorig[3].ravel(), fill_value=0.0
        )
        print("duck3")
        sys.stdout.flush()
        result3 = varinterpolatedfun3(ptsnew)  # ((Vmetric[1], Vmetric[2], Vmetric[3]))
        print(
            "results: Vmetric: %g %g %g  :: Vorig3: %g\n"
            % (
                Vmetric[1, 20, 15, 20],
                Vmetric[2, 20, 15, 20],
                Vmetric[3, 20, 15, 20],
...
def reinterp3dspc_opt_all(Vorig, Vmetric, rho, ug, uu, B, gdetB=None):
    ...
    start_time = datetime.now()
    print(
        "reinterp3dspc_opt_all(start) time elapsed: %d"
        % (datetime.now() - start_time).seconds
    )
    sys.stdout.flush()
    #
    ...
    tkorigarray = kofphfloat(
        faketk, Vmetric[3, 0, 0, :], Vorig[3].view().reshape(-1)
    ).reshape((nx, ny, nz))
    # integerize
    ...
    #
    print(
        "reinterp3dspc_opt_all(done with tk) time elapsed: %d"
        % (datetime.now() - start_time).seconds
    )
    sys.stdout.flush()
    #
    ...
    if 1 == 1:
        ...
        for kk in np.arange(0, nz):
            for jj in np.arange(0, ny):
                for ii in np.arange(0, nx):
                    ...
                    tjorigarray[ii, jj, kk] = jofhfloat(
                        faketj[:],
                        Vmetric[2, ii, :, inttkorigarray[ii, jj, kk]],
                        Vorig2array,
                    )
        #
        ...
        #
        print(
            "reinterp3dspc_opt_all(done with tjorigarray) time elapsed: %d"
            % (datetime.now() - start_time).seconds
        )
        sys.stdout.flush()
        #
        rhoi[:, :, :] = rho[
            faketi[:, None, None], inttjorigarray[:, :, :], inttkorigarray[:, :, :]
        ]
        ugi[:, :, :] = ug[
            faketi[:, None, None], inttjorigarray[:, :, :], inttkorigarray[:, :, :]
        ]
        #
        print(
            "reinterp3dspc_opt_all(done with rho,ug) time elapsed: %d"
            % (datetime.now() - start_time).seconds
        )
        sys.stdout.flush()
        #
        ...
        if 1 == 1:
            dxdxpi = np.copy(uu)
            dxdxpi[0, :, :, :] = dxdxp[
                1, 1, faketi[:, None, None], inttjorigarray[:, :, :], 0
            ]  # inttkorigarray[:,:,:]]
            dxdxpi[1, :, :, :] = dxdxp[
                1, 2, faketi[:, None, None], inttjorigarray[:, :, :], 0
            ]  # inttkorigarray[:,:,:]]
            dxdxpi[2, :, :, :] = dxdxp[
                2, 1, faketi[:, None, None], inttjorigarray[:, :, :], 0
            ]  # inttkorigarray[:,:,:]]
            dxdxpi[3, :, :, :] = dxdxp[
                2, 2, faketi[:, None, None], inttjorigarray[:, :, :], 0
            ]  # inttkorigarray[:,:,:]]
        #
        print(
            "reinterp3dspc_opt_all(done with dxdxp11,12,21,22) time elapsed: %d"
            % (datetime.now() - start_time).seconds
        )
        sys.stdout.flush()
        #
        ...
        #
        print(
            "reinterp3dspc_opt_all(done with uu[0-4]) time elapsed: %d"
            % (datetime.now() - start_time).seconds
        )
        sys.stdout.flush()
        #
        ...
        #
        print(
            "reinterp3dspc_opt_all(done with B[1-4]) time elapsed: %d"
            % (datetime.now() - start_time).seconds
        )
        sys.stdout.flush()
        #
        if gdetB != None:
            ...
            #
            print(
                "reinterp3dspc_opt_all(done with gdetB[1-4]) time elapsed: %d"
                % (datetime.now() - start_time).seconds
            )
            sys.stdout.flush()
            #
    ...
    if 1 == 0:
        #
        print(
            "reinterp3dspc_opt_all(start fast but slower interp) time elapsed: %d"
            % (datetime.now() - start_time).seconds
        )
        sys.stdout.flush()
        #
        for kk in np.arange(0, nz):
            for jj in np.arange(0, ny):
                for ii in np.arange(0, nx):
                    ...
                    if 0 == 1:
                        tkorig = kofphfloat(
                            faketk, Vmetric[3, ii, jj, :], Vorig[3, ii, jj, kk]
                        )
                    ...
                    if 0 == 1:
                        tiorig = iofrfloat(
                            faketi, Vmetric[1, :, jj, inttkorig], Vorig[1, ii, jj, kk]
                        )
                    ...
                    if 0 == 1:
                        ...
                        Vorig2array = np.array([Vorig[2, ii, jj, kk]])
                        tjorig = jofhfloat(
                            faketj, Vmetric[2, inttiorig, :, inttkorig], Vorig2array
                        )
                        # inttjorig=tjorig.astype(int)
                    ...
                    if gdetB != None:
                        ...
                        gdetBi[1, ii, jj, kk] = gdetB[
                            1, inttiorig, inttjorig, inttkorig
                        ]
                        gdetBi[2, ii, jj, kk] = gdetB[
                            2, inttiorig, inttjorig, inttkorig
                        ]
                        gdetBi[3, ii, jj, kk] = gdetB[
                            3, inttiorig, inttjorig, inttkorig
                        ]
                    #
                #
            #
        #
        print(
            "reinterp3dspc_opt_all(end fast but slower interp) time elapsed: %d"
            % (datetime.now() - start_time).seconds
        )
        sys.stdout.flush()
        #
...
def reinterpxy(vartointerp, extent, ncell, domask=1, interporder="cubic"):
    ...
    if domask != 0:
        interior = (
            np.sqrt((xi[None, :] ** 2) + (yi[:, None] ** 2))
            < (1 + np.sqrt(1 - a**2)) * domask
        )
...
def get_RT_seedpoints(fnumber, ncell):
    grid3d(
        "gdump.bin", use2d=True
    )  # load the gdump.bin file - use2d=True to save memory
    ...
    imyz[imyz.mask == True] = np.sqrt(
        rhor**2 - imyx[imyz.mask == True] ** 2 - imyy[imyz.mask == True] ** 2
    )
    ...
    iBr = reinterpxyhor(
        Bznorm, extent, ncell, domask=1, interporder="linear"
    )  # Brnorm is spherical, BRnorm is cylindrical; switching to Bznorm to keep transition to the disk continuous, has sign info

    ahor = (
        2.0 * np.pi * (a**2 + 3 * rhor**2) / 3
    )  # surface area of half of the horizon
    BzH = (
        5.0 / ahor
    )  # Magnetic field per unit area on BH horizon, calculated from Upsilon value in Avara 2015
    Bzfake = (
        iBz * np.sqrt(imyx**2 + imyy**2) / (rhor * np.sqrt(5.75))
    )  # vertical magnetic field in the disk per unit area with radial dependence correction, Mdot=5.75 as in Avara 2015
    Br = iBr / np.sqrt(
        5.75
    )  # radial magnetic field on the horizon per unit area, Mdot=5.75 as in Avara 2015
    Bzfake[iBz.mask == True] = Br[
        iBz.mask == True
    ]  # replace values that are inside the black hole in the equatorial plane with those in the upper half of the horizon
    Bzfake1 = Bzfake / BzH
    ...
    bmax = np.max(
        Bp_slice
    )  # value where the probability distribution goes to 1; value before 7/16 was 1.0
    sl = 1 / (bmax - bmin)
    yint = -bmin * sl
    Bp_slice = (
        sl * Bp_slice + yint
    )  # linear probability distribution between Bp=10 and Bp=100
    ...
    nsp = int(
        max(round(30 * min(ptot / prbmax, 1), 0), 15)
    )  # m=30 is the maximum number of points
    ...
    if previousseed == 1:
        ...
        for n in range(0, lnc):
            ...
            for i in range(1, N + 1):
                exec(
                    "x%s[i,0],x%s[i,1]=propagate_seeds(x%s[i-1,0],x%s[i-1,1],ivx0, ivy0,ax,ay,grid,N,delta_t,ncell)"
                    % (n, n, n, n)
                )
            ...
            if prob_array[n] <= 0:
                ...
                csnew = get_coordsw(
                    xmsk, ymsk, zmsk.data, Bprob, 1, fnumber, previousseed
                )
                newsamp[n, :] = csnew[0, :]
                iofx = getnearpos(grid, newsamp[n, 0])
                jofy = getnearpos(grid, newsamp[n, 1])
        ...
        if lnc > nsp:
            ...
            spn = nsp - lnc
            csplus = get_coordsw(
                xmsk, ymsk, zmsk.data, Bprob, spn, fnumber, previousseed
            )
        ...
        coords_sampled = get_coordsw(
            xmsk, ymsk, zmsk.data, Bprob, nsp, fnumber, previousseed
        )
...
def checkifseedpointexists(fnumber):
    ...
    print("checkifseedpointexists(): checking for fname=%s" % (fname))
    sys.stdout.flush()
    #
    if os.path.isfile(fname):
        print("File %s exists" % fname)
...
def propagate_seeds(xold, yold, ivx, ivy, ax, ay, grid, N, t, ncell):
    ...
    if (
        iofxold == 0
        or iofxold == int(ncell - 1)
        or jofyold == 0
        or jofyold == int(ncell - 1)
    ):
...
def addhorcirc(fnumber, ncell):
    ...
    Bp_slice = (
        sl * Bp_slice + yint
    )  # linear probability distribution between Bz=0.1 and Bz=1.0
    Bp_slice[Bp_slice > 1.0] = 1.0
    Bp_slice[Bp_slice < 0.0] = 0.0
    grid = np.linspace(-rng, rng, ncell)
    ...
    numer = (
        intangle(
            gdet
            * jabs(-bu[1] * np.sqrt(gv3[1, 1]) * bd[3] * np.sqrt(gn3[3, 3]))
            * denfactor,
            doavgn3=0,
            **keywordsrhosq,
        )
    ...
    denom = (
        intangle(
            gdet * (bsq * 0.5 + (gam - 1.0) * ug) * denfactor,
            doavgn3=0,
            **keywordsrhosq,
        )
        / rhosqint
...
def stressvtime(fnumber):
    ...
    nummagzp_in = np.sum(bubble_zp[nxin:nxout, ny / 2 : mhout, :]) - np.sum(
        bubble_zp[nxin:nxout, mhin : ny / 2, :]
    )
    nummagzp_out = np.sum(disk_zp[nxin:nxout, ny / 2 : mhout, :]) - np.sum(
        disk_zp[nxin:nxout, mhin : ny / 2, :]
    )
    ...
    nummagzp_pert_in = np.sum(bubble_zp_pert[nxin:nxout, ny / 2 : mhout, :]) - np.sum(
        bubble_zp_pert[nxin:nxout, mhin : ny / 2, :]
    )
    nummagzp_pert_out = np.sum(disk_zp_pert[nxin:nxout, ny / 2 : mhout, :]) - np.sum(
        disk_zp_pert[nxin:nxout, mhin : ny / 2, :]
    )
    ...
    nummagzp = np.sum(integrand_zp[nxin:nxout, mhin:mhout, :]) - np.sum(
        integrand_zp[nxin:nxout, mhin : ny / 2, :]
    )
    nummagzp_pert = np.sum(integrand_zp_pert[nxin:nxout, ny / 2 : mhout, :]) - np.sum(
        integrand_zp_pert[nxin:nxout, mhin : ny / 2, :]
    )
    ...
    return (
        fnumber,
        alphamagrp_bubble,
        alphamagrp_pert_bubble,
        alphamagrp_disk,
        alphamagrp_pert_disk,
        alphamagzp_bubble,
        alphamagzp_pert_bubble,
        alphamagzp_disk,
        alphamagzp_pert_disk,
    )
    # return ibeta, rho
...
def stressdecompvtime(fnumber):
    ...
    bz = -bu[2] * np.sqrt(
        gv3[2, 2]
    )  # minus sign because theta hat points in -z hat direction
    ...
    integrand_rp = -br * bphi * gdet * _dx1 * _dx2 * _dx3
    integrand_rp_mean = (
        -brmean * bphimean * gdet * _dx1 * _dx2
    )  # avg terms are 2d (r and theta), so don't integrate over phi
    ...
    integrand_zp = -bz * bphi * gdet * _dx1 * _dx2 * _dx3
    integrand_zp_mean = (
        -bzmean * bphimean * gdet * _dx1 * _dx2
    )  # avg terms are 2d (r and theta), so don't integrate over phi
    ...
    nummagzp_bub_tot = np.sum(bubble_zp_tot[nxin:nxout, ny // 2 : mhout, :]) - np.sum(
        bubble_zp_tot[nxin:nxout, mhin : ny // 2, :]
    )
    nummagzp_disk_tot = np.sum(disk_zp_tot[nxin:nxout, ny // 2 : mhout, :]) - np.sum(
        disk_zp_tot[nxin:nxout, mhin : ny // 2, :]
    )
    ...
    nummagzp_bub_cross1 = np.sum(
        bubble_zp_cross1[nxin:nxout, ny // 2 : mhout, :]
    ) - np.sum(bubble_zp_cross1[nxin:nxout, mhin : ny // 2, :])
    nummagzp_disk_cross1 = np.sum(
        disk_zp_cross1[nxin:nxout, ny // 2 : mhout, :]
    ) - np.sum(disk_zp_cross1[nxin:nxout, mhin : ny // 2, :])
    ...
    nummagzp_bub_cross2 = np.sum(
        bubble_zp_cross2[nxin:nxout, ny // 2 : mhout, :]
    ) - np.sum(bubble_zp_cross2[nxin:nxout, mhin : ny // 2, :])
    nummagzp_disk_cross2 = np.sum(
        disk_zp_cross2[nxin:nxout, ny // 2 : mhout, :]
    ) - np.sum(disk_zp_cross2[nxin:nxout, mhin : ny // 2, :])
    # both turbulent
    bubble_zp_pert = ma.masked_where(ibeta < 40, integrand_zp_pert)
    disk_zp_pert = ma.masked_where(ibeta >= 40, integrand_zp_pert)
    nummagzp_bub_pert = np.sum(bubble_zp_pert[nxin:nxout, ny // 2 : mhout, :]) - np.sum(
        bubble_zp_pert[nxin:nxout, mhin : ny // 2, :]
    )
    nummagzp_disk_pert = np.sum(disk_zp_pert[nxin:nxout, ny // 2 : mhout, :]) - np.sum(
        disk_zp_pert[nxin:nxout, mhin : ny // 2, :]
    )
    ...
    nummagzp_tot = np.sum(integrand_zp[nxin:nxout, mhin:mhout, :]) - np.sum(
        integrand_zp[nxin:nxout, mhin : ny // 2, :]
    )
    nummagzp_mean = np.sum(integrand_zp_mean[nxin:nxout, mhin:mhout, :]) - np.sum(
        integrand_zp_mean[nxin:nxout, mhin : ny // 2, :]
    )
    nummagzp_cross1 = np.sum(
        integrand_zp_cross1[nxin:nxout, ny // 2 : mhout, :]
    ) - np.sum(integrand_zp_cross1[nxin:nxout, mhin : ny // 2, :])
    nummagzp_cross2 = np.sum(
        integrand_zp_cross2[nxin:nxout, ny // 2 : mhout, :]
    ) - np.sum(integrand_zp_cross2[nxin:nxout, mhin : ny // 2, :])
    nummagzp_pert = np.sum(integrand_zp_pert[nxin:nxout, ny // 2 : mhout, :]) - np.sum(
        integrand_zp_pert[nxin:nxout, mhin : ny // 2, :]
    )
...
def tutorial1alt():
    ...
    nxout = int(iofr(30))
    myx = (
        r[nxin:nxout, :, 0] * np.sin(h[nxin:nxout, :, 0]) * np.cos(ph[nxin:nxout, :, 0])
    )
    myy = (
        r[nxin:nxout, :, 0] * np.sin(h[nxin:nxout, :, 0]) * np.sin(ph[nxin:nxout, :, 0])
    )
